...
4. **Artifact graph registration is centralized manually.** `Renderer.cpp` registers producers individually, while requests are spread across managers/services. A declarative producer catalog could expose kind, lane, domain, input/output types and supported milestones for documentation and validation.
