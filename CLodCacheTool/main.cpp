...
static const char* NodeKindName(uint32_t kind) {
    switch (kind) {
    case 0u: return "internal";
    case 1u: return "voxel-leaf";
...
static void WriteClusterLODReport(
    ...
    for (uint32_t depth = 0; depth < static_cast<uint32_t>(data.lodLevelRoots.size()); ++depth) {
        ...
        out << depth << ","
            ...
            << (root && root->range.isGroup == 0u ? root->range.countMinusOne + 1u : 0u) << ","
    ...
    out << "\nTop root children:\n";
    out << "child_index,node_id,kind,child_count,error,lod_radius,owner_group\n";
    if (!nodes.empty() && nodes[0].range.isGroup == 0u) {
        ...
        for (uint32_t child = 0; child < childCount; ++child) {
            ...
            out << child << ","
                ...
                << (node.range.isGroup == 0u ? node.range.countMinusOne + 1u : 0u) << ","
                << node.traversalMetric.maxQuadricError << ","
                << node.traversalMetric.lodBoundingSphere.w << ","
                << (node.range.isGroup == 0u ? -1 : static_cast<int32_t>(node.range.ownerGroupId)) << "\n";
        }
    }
