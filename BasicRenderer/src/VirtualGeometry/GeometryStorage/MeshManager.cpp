...
#include "VirtualGeometry/GeometryStorage/MeshManagerCLodGeometryStorage.h"
...
bool MeshManager::AddMesh(std::shared_ptr<Mesh>& mesh, bool useMeshletReorderedVertices) {
	...
	// Create shared streaming state (once per mesh, before hierarchy CPU data is released)
	{
...

		{
			...
			PublishCLodStreamingDomainEventForSharedState(br::render::CLodStreamingDomainEventKind::SharedMeshAdded, sharedState);
		}
	}
...
void MeshManager::PrepareStaticMeshTemplateResourcesAsync(const std::vector<br::render::StaticMeshTemplateRequest>& requests) {
	std::lock_guard publicationLock(m_staticTemplatePublicationMutex);
...
std::vector<br::render::StaticMeshTemplateRegistration> MeshManager::AddStaticMeshTemplatesBulk(const std::vector<br::render::StaticMeshTemplateRequest>& requests) {
	std::lock_guard publicationLock(m_staticTemplatePublicationMutex);
	ZoneScopedN("MeshManager::AddStaticMeshTemplatesBulk");
	ZoneValue(static_cast<int64_t>(requests.size()));
	std::vector<br::render::StaticMeshTemplateRegistration> registrations(requests.size());
	if (requests.empty()) {
		...
		for (size_t rowIndex = 0; rowIndex < validRequestIndices.size(); ++rowIndex) {
			...
			if (sharedState) {
				...
				if (sharedState->groupCount > 0u) {
					...
					if (wasInactive) {
						PublishCLodStreamingDomainEventForSharedState(br::render::CLodStreamingDomainEventKind::ActiveRangeAdded, sharedState);
					}
				}
			}
		}
		...
		if (itState != m_clodStreamingStateByInstanceIndex.end()) {
			...
			if (sharedMeshState != nullptr && sharedMeshState->activeInstanceCount > 0u) {
				...
				if (sharedMeshState->activeInstanceCount == 0u) {
					...
					m_clodSharedStreamingRangesDirty = true;
					PublishCLodStreamingDomainEventForSharedState(br::render::CLodStreamingDomainEventKind::ActiveRangeRemoved, sharedMeshState);
...
void MeshManager::ProcessCLodDiskStreamingIO() {
	...
	std::vector<br::render::CLodDiskStreamingCompletion> newCompletions;
...
void MeshManager::PublishCLodStreamingDomainEvent(br::render::CLodStreamingDomainEvent event) {
	if (event.groupCount == 0u && event.kind != br::render::CLodStreamingDomainEventKind::FullReset) {
		return;
	}
...
void MeshManager::PublishGeometryResidencyDelta(const br::render::CLodStreamingDomainEvent& event) {
	if (event.kind == br::render::CLodStreamingDomainEventKind::SharedMeshAdded) return;
	std::lock_guard lock(m_geometryResidencyPublicationMutex);
	...
	case br::render::CLodStreamingDomainEventKind::ActiveRangeRemoved:
		...
		break;
	case br::render::CLodStreamingDomainEventKind::FullReset:
		input->kind = br::render::GeometryResidencyDeltaKind::Reset;
		break;
...
void MeshManager::PublishCLodStreamingDomainEventForSharedState(
	br::render::CLodStreamingDomainEventKind kind,
	const std::shared_ptr<CLodSharedStreamingState>& sharedState) {
	if (sharedState == nullptr || sharedState->groupCount == 0u) {
		return;
	}

	br::render::CLodStreamingDomainEvent event{};
	event.kind = kind;
	...
	for (const auto& localRange : sharedState->coarsestRanges) {
		...
		br::render::CLodActiveGroupRange range{};
...
void MeshManager::DrainCLodStreamingDomainEvents(std::vector<br::render::CLodStreamingDomainEvent>& outEvents, uint64_t& outGeneration) {
	outEvents.clear();
	{
		std::lock_guard lock(m_clodStreamingDomainEventsMutex);
...
MeshManager::DiskStreamingApplyResult MeshManager::PrepareCompletedCLodDiskStreamingResult(
	...
	br::render::CLodDiskStreamingCompletion& outCompletion) {
	outCompletion = {};
	...
	if (result.directStorageGpuUploadPending) {
		...
		if (launch.fetchedPageCount == 0u) {
			...
			outCompletion.payloadKind = br::render::CLodDiskStreamingPayloadKind::ReusedExistingPages;
	...
	if (result.mappedContainer != nullptr &&
		...
		outCompletion.payloadKind =
			br::render::CLodDiskStreamingPayloadKind::CpuMappedPageViews;
		outCompletion.chunk = chunk;
	...
	outCompletion.groupGlobalIndex = result.groupGlobalIndex;
	outCompletion.success = true;
	outCompletion.payloadKind = br::render::CLodDiskStreamingPayloadKind::CpuPageBlobs;
	outCompletion.chunk = chunk;
...
void MeshManager::FinalizePendingCLodDirectStorageUploads(
	...
	std::vector<br::render::CLodDiskStreamingCompletion>& outCompletions,
...
void MeshManager::DrainCompletedCLodDiskStreamingGroups(std::vector<br::render::CLodDiskStreamingCompletion>& outCompletions) {
	std::lock_guard<std::mutex> resultsLock(m_clodDiskStreamingResultsMutex);
...
bool MeshManager::LaunchPendingCLodDirectStorageUploads(rhi::Timeline waitTimeline, uint64_t waitValue) {
	...
	std::vector<br::render::CLodDiskStreamingCompletion> failedCompletions;
...
bool MeshManager::QueueCLodGroupDiskIO(uint32_t groupGlobalIndex, const std::vector<bool>& segmentNeedsFetch, const std::vector<uint32_t>& preAllocatedPages, uint32_t priority, const CLodCache::GroupPayloadLayoutMetadata* prefetchedLayout) {
	...
	std::vector<br::render::CLodGroupDiskIOBatchRequest> requests;
...
uint32_t MeshManager::QueueCLodGroupDiskIOBatch(const std::vector<br::render::CLodGroupDiskIOBatchRequest>& requests, std::vector<bool>* outQueuedByRequest) {
...
br::render::CLodGroupStreamingInfo MeshManager::GetCLodGroupStreamingInfo(uint32_t groupGlobalIndex) const {
	br::render::CLodGroupStreamingInfo info{};
	...
	if (localIndex < sharedState->groups.size()) {
		...
		if (firstSegment < sharedState->segments.size()) {
			...
			for (uint32_t segmentOffset = 0u; segmentOffset < static_cast<uint32_t>(info.segments.size()); ++segmentOffset) {
				...
				br::render::CLodGroupStreamingInfo::ReferencedPageSegment referenced{};
...
void MeshManager::GetCLodActiveUniqueAssetGroupRanges(std::vector<br::render::CLodActiveGroupRange>& outRanges, uint32_t& outMaxGroupIndex) const {
	std::lock_guard publicationLock(m_staticTemplatePublicationMutex);
	outRanges.clear();
	outMaxGroupIndex = 0u;
...
void MeshManager::GetCLodCoarsestUniqueAssetGroupRanges(std::vector<br::render::CLodActiveGroupRange>& outRanges) const {
	std::lock_guard publicationLock(m_staticTemplatePublicationMutex);
	outRanges.clear();
...
void MeshManager::GetCLodStreamingDomainSnapshot(br::render::CLodStreamingDomainSnapshot& outSnapshot) const {
	std::lock_guard publicationLock(m_staticTemplatePublicationMutex);
	...
	for (const auto& [_, state] : m_clodStreamingStateByInstanceIndex) {
		...
		br::render::CLodActiveGroupRange activeRange{};
		...
		for (const auto& localRange : state.sharedMeshState->coarsestRanges) {
			...
			br::render::CLodActiveGroupRange coarsest{};
...
br::render::CLodStreamingDebugStats MeshManager::GetCLodStreamingDebugStats() const {
	br::render::CLodStreamingDebugStats stats{};
...
void MeshManager::GetCLodRayTracingResidencySnapshot(br::render::CLodRayTracingResidencySnapshot& outSnapshot) const {
	...
	for (const CLodSharedStreamingRange& range : m_clodSharedStreamingRanges) {
		...
		for (uint32_t localGroupIndex = 0; localGroupIndex < groupCount; ++localGroupIndex) {
			...
			br::render::CLodRayTracingResidentGroup rtGroup{};
