...
std::function<void(std::uint64_t)> RendererStateRequestService::MakeSuspensionNotifier() {
    return m_graph.MakeSuspensionNotifier();
}

void RendererStateRequestService::RegisterProducer(ArtifactKind kind, ArtifactProducerRegistration registration) {
        m_graph.RegisterProducer(kind, std::move(registration));
    }


void RendererStateRequestService::Release(ArtifactKey key) { m_graph.Release(key); }

void RendererStateRequestService::ReleaseBatch(std::span<const ArtifactKey> keys) { m_graph.ReleaseBatch(keys); }

ArtifactSnapshot RendererStateRequestService::Snapshot(ArtifactAddress address) const {
        return m_graph.Snapshot(address);
    }

ArtifactSnapshot RendererStateRequestService::Snapshot(ArtifactVersionID version) const {
        return m_graph.Snapshot(version);
    }

ArtifactObservation RendererStateRequestService::ObserveWithSnapshot(ArtifactAddress address,
...
ArtifactObservation RendererStateRequestService::ObserveKind(ArtifactKind kind,
		std::function<void(std::uint64_t, const ArtifactSnapshot&)> callback) {
		return m_graph.ObserveKind(kind, std::move(callback));
	}

ArtifactAwaiter RendererStateRequestService::AwaitExact(ArtifactVersionHandle handle,
...
AsyncStateGraphStats RendererStateRequestService::Stats() const { return m_graph.Stats(); }

std::uint64_t RendererStateRequestService::Outstanding(ArtifactKind kind) const {
        return m_graph.Outstanding(kind);
    }

bool RendererStateRequestService::TraceActive() const { return m_graph.TraceActive(); }

void RendererStateRequestService::TraceEvent(AsyncStateGraphTraceEventID event, ArtifactAddress address,
        std::uint64_t revision, std::uint64_t generation,
        AsyncStateGraphTracePayload payload, ArtifactAddress related,
        std::uint64_t relatedRevision) {
        m_graph.TraceEvent(event, address, revision, generation, payload,
            related, relatedRevision);
    }

std::uint64_t RendererStateRequestService::SubscribeReady(
        std::function<void(const ArtifactSnapshot&)> callback) {
        return m_graph.AddReadyCallback(std::move(callback));
    }

void RendererStateRequestService::UnsubscribeReady(std::uint64_t subscription) {
