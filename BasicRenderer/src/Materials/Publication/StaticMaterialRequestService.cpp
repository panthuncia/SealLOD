...
MaterialUsageCapture StaticMaterialRequestService::CaptureMaterialUsage(Material& material, unsigned int count, bool refresh) const { return Storage(m_materials).CaptureMaterialUsage(material, count, refresh); }
std::shared_ptr<const MaterialUsageReservation> StaticMaterialRequestService::ReserveMaterialUsage(const std::vector<MaterialUsageCapture>& captures) const { return Storage(m_materials).ReserveMaterialUsage(captures); }
