...
const org::PipelineState& PSOManager::GetShadowMeshPSO(UINT psoFlags, MaterialCompileFlags materialCompileFlags, bool wireframe) {
    ...
    return FindOrCreatePipelineState(m_cacheMutex, m_shadowMeshPSOCache, key, [&]() {
        return RegisterPipeline(
            ...
            "Material.ShadowMesh",
            br::extensions::PipelineKind::Mesh,
            [this, psoFlags, materialCompileFlags, wireframe] {
...
org::PipelineState PSOManager::BuildComputePipeline(const ComputeRecipe& recipe, const br::diagnostics::RecompileOptions* options)
{
...
org::PipelineState PSOManager::RegisterComputePipeline(org::PipelineState state, ComputeRecipe recipe)
{
    ...
    entry.kind = br::extensions::PipelineKind::Compute;
    entry.state = state;
...
std::vector<br::diagnostics::LivePipelineInfo> PSOManager::ListPipelines() const
{
    std::scoped_lock lock(m_livePipelineMutex);
    std::vector<br::diagnostics::LivePipelineInfo> result;
    result.reserve(m_livePipelines.size());
    for (const auto& [_, entry] : m_livePipelines) {
        const auto payload = entry.state.GetPayload();
        br::diagnostics::LivePipelineInfo info;
        info.id = entry.id;
    ...
    std::ranges::sort(result, {}, &br::diagnostics::LivePipelineInfo::id);
    return result;
}

std::optional<br::diagnostics::LiveJobInfo> PSOManager::GetLiveJob(uint64_t jobId) const
{
    std::scoped_lock lock(m_livePipelineMutex);
    const auto it = m_liveJobs.find(jobId);
    return it == m_liveJobs.end() ? std::nullopt : std::optional<br::diagnostics::LiveJobInfo>{ it->second };
}

uint64_t PSOManager::RequestRecompile(const std::string& pipelineId, br::diagnostics::RecompileOptions options)
{
    ComputeRecipe recipe;
    std::function<org::PipelineState()> rebuild;
    ...
    const uint64_t jobId = m_nextLiveJobId.fetch_add(1, std::memory_order_relaxed);
    {
        ...
        m_liveJobs.emplace(jobId, br::diagnostics::LiveJobInfo{
            .id = jobId,
            .pipelineId = pipelineId,
            .state = br::diagnostics::LiveJobState::Queued
        });
    }
        ...
        [this,
            ...
            options = std::move(options)]() mutable {
            {
                ...
                if (auto it = m_liveJobs.find(jobId); it != m_liveJobs.end()) {
                    it->second.state = br::diagnostics::LiveJobState::Compiling;
                }
            }
            try {
                ...
                job.state = br::diagnostics::LiveJobState::ReadyToPublish;
                ...
                auto& job = m_liveJobs.at(jobId);
                job.state = br::diagnostics::LiveJobState::Failed;
...
uint64_t PSOManager::RequestActivation(const std::string& pipelineId, uint64_t generation)
{
    ...
    m_liveJobs.emplace(jobId, br::diagnostics::LiveJobInfo{
        .id = jobId,
        .pipelineId = pipelineId,
        .state = br::diagnostics::LiveJobState::ReadyToPublish,
...
void PSOManager::PublishPendingLivePipelines(std::vector<PipelineRetirementPoint> retirementPoints)
{
    ...
    while (!m_pendingLivePublications.empty()) {
        ...
        entry.compiling = false;
        job.state = br::diagnostics::LiveJobState::Published;
        m_pipelineEpoch.fetch_add(1, std::memory_order_acq_rel);
        spdlog::info(
...
br::diagnostics::PipelineGenerationSnapshot PSOManager::GetPipelineGenerationSnapshot() const
{
    br::diagnostics::PipelineGenerationSnapshot snapshot;
    snapshot.epoch = GetPipelineEpoch();
    snapshot.pipelines = ListPipelines();
    uint64_t digest = 14695981039346656037ull;
    for (const br::diagnostics::LivePipelineInfo& pipeline : snapshot.pipelines) {
